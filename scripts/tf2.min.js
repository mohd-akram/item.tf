...
'%20Set"\n target="_blank">\n<div class="rounded glow" style="display: inline-block; padding: 7px">\nView Items\n</div>\n</a>':""};b.prototype._priceSourceHTML=function(){var a,c,b,g,e,f,h;c="";this.item.prices[this.source]||this._nextPriceSource();if(this.item.prices[this.source]){g=Object.keys(this.item.prices[this.source]).sort();f=0;for(h=g.length;f<h;f++){e=g[f];b=this.item.prices[this.source][e];if(a=b.match(/(Refined|Key(s)?|Bud(s)?)/))a=a[0],b=b.replace(/(\d+(\.\d+)?)/g,'<a href="/search?q=$1%20'+
a+'"\n target="_blank" class="glow">$1</a>');c+="<span class='"+e.toLowerCase()+"'>"+e+"</span>: "+b+"<br>"}}return c};b.prototype._pricesHTML=function(){var a;return(a=this._priceSourceHTML())?"<div id='marketprice'><span id='pricesource'>"+m(this.source)+"</span><h3 id='prices'>"+a+"</h3></div>":""};b.prototype._blueprintsHTML=function(){var a,c,b,g,e,f,h,l,q,k,n,m,p,r;if(this.item.blueprints.length){b='<div id="blueprints">';m=this.item.blueprints;h=0;for(k=m.length;h<k;h++){a=m[h];c=a.getAttribute("data-chance");
